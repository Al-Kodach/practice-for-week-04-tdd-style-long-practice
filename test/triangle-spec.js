...
describe('Triangle Class', () => {
   ...
   describe('hasValidSideLengths instance method', () => {
      context("If the sum of any two sides is greater than the remaining side", () => {
         it('Should return true', () => {
            expect(triangele.hasValidSideLengths()).to.be.true;
         });
      });

      context('If the sum of any two sides is less than the remaining side', () => {
   ...
   describe('getValidTriangles static method0', () => {
      it('Should return all valid triangle(s)', () => {
         ...
         let expected1 = Triangle.getValidTriangles([ triangele, triangele2, triangele3 ]);
...
describe('Scalene class', () => {
   let scaleneTriangle;
   beforeEach(() => {
      scaleneTriangle = new Scalene(8, 6, 4);
   });
   ...
   it('Should be initialized with 3 side lengths', () => {
      expect(scaleneTriangle.side1).to.equal(8);
      expect(scaleneTriangle.side2).to.equal(6);
      expect(scaleneTriangle.side3).to.equal(4);
   });
   ...
   describe('validate instance property', function () {
      it('Should over-write the Triangle validate intance methoed',
      function () {
...
describe('Isosceles class', () => {
   ...
   it('Should be initialized with 3 side lengths', () => {
      expect(isosceles.side1).to.equal(6);
      expect(isosceles.side2).to.equal(6);
      expect(isosceles.side3).to.equal(4);
   });

   it('Should hava an `isValidTriangle` property, with a value derived from the `Triangle` class `hasValidSideLengths` method',
   () => {
      expect(isosceles).to.have.property('isValidTriangle');
      expect(isosceles.isValidTriangle).to.be.true;
   });

   describe('isIsosceles instance method', () => {
      it('Should have an instance method isIsosceles', () => {
         const spyIsScalene = chai.spy(isosceles, 'isIsosceles');
         isosceles.isIsosceles();

         expect(spyIsScalene).to.be.called;
      });

      it('Should return true if two sides have the same lengths',
      () => {
         expect(isosceles.isIsosceles()).to.equal(true);
      });

      it('Should return false if two or all sides have different length',
      () => {
         let isosceles1 = new Isosceles(5, 4 , 8);
         expect(isosceles1.isIsosceles()).to.equal(false);
      });
   });
   ...
   describe('validate instance property', function () {
      it('Should over-write the Triangle validate intance methoed',
      function () {
         const spy = chai.spy(isosceles, 'isValidIsosceles');

         isosceles.isValidIsosceles();
         expect(spy).to.be.called;
